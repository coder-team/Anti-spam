...
local function plugin_exists( name )
  ...
  local function list_plugins(only_enabled)
    ...
    for k, v in pairs(plugins_names()) do
      ...
      if not only_enabled or status == 'ðŸ”˜' then
        -- get the name
        v = v:match('(.*)%.lua')
        text = text..status..'  '..v..'\n'
      end
    end
    local text = text..'âž–âž–âž–âž–âž–âž–\n'..psum..'  number plugin. \nðŸ”˜  '
...
local function enable_plugin( plugin_name )
  ...
  if plugin_enabled(plugin_name) then
    return 'Plugin[ '..plugin_name..' ] Was Active.ðŸ”˜'
  end
  ...
  if plugin_exists(plugin_name) then
    ...
    save_config()
    -- Reload the plugins
    return 'Plugin[ '..plugin_name..' ]Active.ðŸ”˜'
  else
